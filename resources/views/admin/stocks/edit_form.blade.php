<!-- Modal -->
<div class="modal fade container-fluid" id="stockEdit{{$stock->id}}" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
                ...
                <form action="{{route('stocks.update', $stock)}}" method="post">
                    ...
                    <div class="container form-inline increment">
                        <div class="form-group mb-2">
                            <label for="selectColor"><span class="badge bg-{{$stock->color->name}}"><ion-icon size="small" name="color-fill-outline"></ion-icon></span></label>
                            <select name="color_id"  class="custom-select ml-2 text-lowercase" disabled>
                                @foreach ($colors as $color)
                                    <option @if ($stock->color->id == $color->id)  
                                        selected                                           
                                    @endif value="{{$color->id}}">{{__($color->name)}}</option>
                                @endforeach
                              </select>
                        </div>
                        <div class="form-group mx-sm-3 mb-2">
                            <div class="nav flex-column nav-tabs" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                                ...
                                <a class="nav-link d-none {{$stock->size->type_sizes_id == $type->id ? ' active' : '' }}" id="edit{{$type->id}}" data-toggle="tab" href="#edit{{$type->name}}"
                                  role="tab" aria-controls="{{$type->name}}" aria-selected="{{$key == 0? 'true' : 'false' }}"></a>
                                @endforeach
                                <select class="form-control" onchange="document.getElementById(`edit${this.value}`).click()" disabled>
                                  @foreach ($type_sizes as $key => $type)
                            ...
                            <div class="tab-content ml-3">
                                @foreach ($type_sizes as $key => $type)
                                <div class="tab-pane fade {{$stock->size->type_sizes_id == $type->id ? 'show active' : ''}}" id="edit{{$type->name}}" role="tabpanel" aria-labelledby="edit{{$type->id}}">                               
                                    <select class="form-control" required disabled onchange="setSize(this.value, 'size_edit')">
