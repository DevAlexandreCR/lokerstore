            ...
            <div class="collapse navbar-collapse w-100 p-0">
                <ul class="flex-md-column flex-row navbar-nav w-100 justify-content-between">
                    <li class="nav-item">
                        ...
                        ><ion-icon class="mr-2" name="home-outline"></ion-icon><span class="font-weight-bold">{{__('My Account')}}</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link pl-0 {{ ! Route::is('user.profile') ?: 'font-weight-bolder'}}"
                           href="{{ route('user.profile', auth()->id()) }}"
                        ><ion-icon class="mr-2" name="person-circle-outline"></ion-icon><span class="d-none d-md-inline">{{__('Profile')}}</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link pl-0 {{ ! Route::is('cart.show') ?: 'font-weight-bolder'}}"
                           href="{{ route('cart.show', auth()->id()) }}"
                        ><ion-icon  class="mr-2" name="cart-outline"></ion-icon><span class="d-none d-md-inline">{{__('Cart')}}</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link pl-0 {{ ! Route::is('user.orders.index') ?: 'font-weight-bolder'}}"
                           href="{{ route('user.orders.index', auth()->id()) }}"
                        ><ion-icon class="mr-2" name="pricetags-outline"></ion-icon><span class="d-none d-md-inline">{{__('Orders')}}</span></a>
                    </li>
                </ul>
            </div>
        </nav>
